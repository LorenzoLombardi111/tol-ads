-- Complete Database Setup for Credit System
...
-- 5. Create user_roles table for admin functionality
...
-- 6. Create function to update user credits
...
-- 7. Create trigger to automatically create user_credits record for new users
...
-- 9. Enable RLS and create simple policies
...
-- Simple policy for user_roles (no recursion)
CREATE POLICY "Allow all operations on user_roles" ON user_roles
    FOR ALL USING (true);

-- 10. Grant initial credits to existing users
INSERT INTO user_credits (user_id, credits_available, credits_used)
SELECT id, 10, 0
FROM auth.users
...
-- 11. Create transaction records for the initial credits
